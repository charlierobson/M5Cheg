...
To help with maintaining patch integrity I defined [a couple of macros](https://github.com/charlierobson/M5Cheg/blob/master/patch.asm) which help to bound the patch code/data. They use BRASS's relocation capability, which compiles to the current PC but uses a virtual PC for calculations involving addresses. Along with a size check and padding for under-sized blocks it makes a nice little feature. You have to specify the patch's address and size as parameters, so there's some manual input, but the benefits are huge. With this in place you can declare labels inside one patch and refer to it in another, which makes table generation and trampolines more readable and safer. 
